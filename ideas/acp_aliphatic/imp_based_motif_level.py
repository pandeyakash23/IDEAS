...
# Get the IDEAS root directory (two levels up from ideas/acp_aliphatic/)
IDEAS_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, '..', '..'))

# Add paths relative to IDEAS root
sys.path.append(os.path.join(IDEAS_ROOT, 'datasets', 'acp_aliphatic'))
sys.path.append(os.path.join(IDEAS_ROOT, 'importance_models', 'acp_aliphatic'))

from generate_property import output_property as output_property_oracle
...
parser = argparse.ArgumentParser(description='Run Adalead with specified temperature for Aliphatic Index optimization.')
parser.add_argument('--temp', type=float, default=1, help='Temperature parameter for Adalead')
parser.add_argument('--device', type=str, default='cuda')
input_args = parser.parse_args()

print(f'========= Aliphatic Index Optimization - Temperature = {input_args.temp} ===========')
...
start_fitness = output_property_oracle(seq_start).tolist()
