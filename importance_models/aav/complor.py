...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.03, max_len: int = 800):
        super().__init__()
...
class complor_network(nn.Module):
    def __init__(self, num_classes, d_model, d_out, max_m, rank):
        ...
        self.cnn1 = nn.Sequential( nn.Conv1d(self.d_model,64,4, stride=1), 
                                   nn.ReLU(),
                                   nn.Conv1d(64,128,4, stride=1),
                                   nn.ReLU(),
                                   nn.Conv1d(128,int(self.d_out),4,stride=1),
                                   ) ##16 size motifs
        self.ln1 = nn.LayerNorm(self.d_out)

        self.maxpool1 = nn.Sequential( nn.AvgPool1d(4, stride=1),
        ...
        for i in range(q_id): # i is related to size of latent rep
            for j in range(d_id): # j in related to num of categorical var
                for prot in range(total): # prot defines the protein number in a batch
                    ...
                    # temp = torch.abs(var_sign*d_comp[prot,j,0:l]*q_comp[prot,i,0:l])
                    # temp = (temp)/(torch.max(temp)+1E-18) #0,1
                    
                    #AKASH CHANGES (for positive importance only)
                    temp_pos = nn.ReLU()(var_sign*d_comp[prot,j,0:l]*q_comp[prot,i,0:l]) # taking abs because the magnitude matters
                    #AKASH CHANGES (for negative importance only)
                    temp_neg = nn.ReLU()(-var_sign*d_comp[prot,j,0:l]*q_comp[prot,i,0:l]) # taking abs because the magnitude matters
                    # normalized pos and neg separately using max
                    temp_pos = (temp_pos)/(torch.max(temp_pos)+1E-18) #0,1
                    temp_neg = (temp_neg)/(torch.max(temp_neg)+1E-18) # 0,1
                    # temp = temp_pos
                    temp = temp_pos - temp_neg # [0, 1]
