...
def predict_property(model, ohe, seq_len, rank):
    test_loader, test_size, _  = make_dataset(ohe, seq_len)
    predicted = torch.zeros((test_size, 1)).to(rank)
    count_test = 0
    ...
    with torch.no_grad():
        for _, (i_x, i_seq) in enumerate(test_loader):
            ...
            predicted[count_test:count_test+i_batch, :] = iter_y_pred
            count_test += i_batch

    return predicted.cpu().numpy().reshape(-1)
