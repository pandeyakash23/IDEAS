...
# Get the IDEAS root directory (two levels up from importance_models/acp_aliphatic/)
...
DATA_PATH = os.path.join(IDEAS_ROOT, 'datasets', 'acp_aliphatic')
...
def make_dataset():
    ohe = np.load(os.path.join(DATA_PATH, 'x_train.npy'), allow_pickle=True)
    classes = np.argmax(ohe, axis=2)
    output = np.load(os.path.join(DATA_PATH, 'y_train.npy'), allow_pickle=True)
    seq_len = np.load(os.path.join(DATA_PATH, 'len_train.npy'), allow_pickle=True)
    ...
    ohe_valid = np.load(os.path.join(DATA_PATH, 'x_valid.npy'), allow_pickle=True)
    classes_valid = np.argmax(ohe_valid, axis=2)
    output_valid = np.load(os.path.join(DATA_PATH, 'y_valid.npy'), allow_pickle=True)
    seq_len_valid = np.load(os.path.join(DATA_PATH, 'len_valid.npy'), allow_pickle=True)
...
def initalize(rank, max_m, init_lr):
    ...
    np.save(os.path.join(MODEL_DIR, 'save_dict.npy'), save_dict)
...
if __name__ == '__main__':
    ...
    init_lr = 0.003
    os.makedirs(MODEL_DIR, exist_ok=True)
    np.save(os.path.join(MODEL_DIR, 'init_lr'), init_lr)
    max_m = int(1)
    ##change
